 ...
 * @author Rose
 */
public class XmlDescription implements LanguageDescription {

    @Override
    public boolean isOperator(char[] characters, int length) {
        if (length == 1) {
            char c = characters[0];
            // XML operators: angle brackets, slash, equals, colon, hyphen, underscore, question mark, exclamation
            return (c == '<' || c == '>' || c == '/' || c == '=' || c == ':' ||
                    c == '-' || c == '_' || c == '?' || c == '!');
        }
        return false;
    ...
    public boolean isLineCommentStart(char a, char b) {
        // XML does not have line comments
        return false;
    }

    @Override
    public boolean isLongCommentStart(char a, char b) {
        // XML comment starts with "<!--" but only two chars are checked here.
    ...
    public boolean isLongCommentEnd(char a, char b) {
        // XML comment ends with "-->" -> first two chars are '-', '-'
        return a == '-' && b == '-';
    }

    @Override
    public String[] getKeywords() {
        // Common XML keywords (e.g., processing instruction, DTD, built-in entities)
        return new String[]{
                "xml", "version", "encoding", "standalone", "CDATA", "DOCTYPE",
                "ELEMENT", "ATTLIST", "ENTITY", "SYSTEM", "PUBLIC"
        };
    }
